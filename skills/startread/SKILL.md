...
이 skill은 Sonnet으로 충분합니다. 메인 세션 모델이 Opus이면 **Sonnet subagent에 위임해 비용 절감**하세요:

```
Agent(
  subagent_type="oh-my-claudecode:executor",
  model="sonnet",
  description="startread: 직전 세션 복원",
  prompt="<수행 순서 전체 + $BRAIN 경로 + 오늘 날짜>"
)
```

subagent가 파일 읽기 + 요약을 수행하고, 메인 Claude는 결과(2~5줄 브리핑)만 사용자에게 전달합니다. 사용자 상호작용 필요 없음.
